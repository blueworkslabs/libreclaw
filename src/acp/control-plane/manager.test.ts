...
describe("AcpSessionManager", () => {
  ...
  it.each(["Resource not found: acpx-sid-stale", "ACP -32602 Invalid params"])(
    "retries once with a fresh persistent session after an early resume turn failure: %s",
    async (resumeError) => {
      const runtimeState = createRuntime();
      hoisted.requireAcpRuntimeBackendMock.mockReturnValue({
        id: "acpx",
      ...
      runtimeState.runTurn
        .mockImplementationOnce(async function* () {
          yield {
            type: "error" as const,
            code: "NO_SESSION",
            message: `Persistent ACP session acpx-sid-stale could not be resumed: ${resumeError}`,
          };
        })
        .mockImplementationOnce(async function* () {
          yield { type: "done" as const };
