...
describe("buildAgentSystemPrompt", () => {
  ...
  it("omits extended sections in minimal prompt mode", () => {
    ...
    expect(prompt).toContain("Prioritize safety and human oversight over completion");
    expect(prompt).toContain("if instructions conflict, pause and ask");
  ...
  it("includes skills in minimal prompt mode when skillsPrompt is provided (cron regression)", () => {
    ...
    expect(prompt).toContain("When a skill drives external API writes, assume rate limits");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("If multiple could apply: choose the most specific one");
    expect(prompt).toContain("Docs: /tmp/openclaw/docs");
    expect(prompt).toContain("OpenClaw behavior/config/architecture: read local docs first.");
  });
